...
app = FastAPI(title="Deep Research AI", debug=True)
...
async def register_page(request: Request):
    try:
        ...
        print(f"Error rendering register page: {e}", file=sys.stderr)
        traceback.print_exc()
        return JSONResponse(status_code=500, content={"detail": f"Render Error: {str(e)}"})
